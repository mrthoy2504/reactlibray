...
const BorrowerDetailSearch = ({
  ...
  const playBeep = () => {
    ...
    oscillator.start();
    oscillator.stop(audioCtx.currentTime + 0.2); // เสียงดัง 0.2 วินาที
    console.log("Beep!");
  };
  ...
  const handleSearch = async (codeId) => {
    try {
      ...
      playBeep(); // เล่นเสียงเมื่อค้นหาสำเร็จ
      inputEl.current.select();
    } catch (err) {
      alert("ไม่พบสมาชิกที่ค้นหา " + err.response.status);
      playBeep(); // เล่นเสียงเมื่อค้นหาไม่สำเร็จ (ถ้าต้องการ)
      setMembers([]);
      setBrowers([]);
      setMembersId("");
    }
  ...
  useEffect(() => {
    const handleKeyDown = (event) => {
      if (
        ...
        handleSearch(codeId); // เรียกฟังก์ชันค้นหา
      }
    };
